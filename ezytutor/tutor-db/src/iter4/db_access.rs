...
pub async fn get_courses_for_tutor_db(pool: &PgPool, tutor_id: i32) -> Result<Vec<Course>, EzyTutorError> {
    ...
    let courses: Vec<Course> = course_rows.iter()
        .map(|crow| Course {
            course_id: crow.course_id,
            course_name: crow.course_name.clone(),
            tutor_id: crow.tutor_id,
            posted_time: Some(chrono::NaiveDateTime::from(crow.posted_time.unwrap())),
        }).collect();

    match courses.len() {
        0 => Err(EzyTutorError::NotFound(
            "Course not found for tutor".into(),
        )),
        _ => Ok(courses)
    }
}
