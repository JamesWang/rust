...
pub async fn get_courses_for_tutor(app_state: web::Data<AppState>, params: web::Path<i32>) -> HttpResponse {
    let tutor_id: i32 = params.abs();
    println!("get tutor {}'s courses", tutor_id );
...
mod tests {
    ...
    async fn get_all_courses_success() {
        ...
        let response = get_courses_for_tutor(get_app_state(), tutor_id).await;
    ...
    async fn get_one_course_success() {
        ...
        let resp = get_course_detail(get_app_state(), params).await;
        assert_eq!(resp.status(), StatusCode::OK);
    }
}